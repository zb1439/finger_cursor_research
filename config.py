...
_config_dict = dict(
    ...
    TRAIN_LOOP=dict(
        EPOCHS=20,
        EVAL_PERIOD=2,  # eval every 1 epoch and dump a checkpoint
        LOG_PERIOD=100,  # log loss/acc every 100 iteration (not epoch)
        DUMP_PATH="",  # where to save the model checkpoint and other metrics
        LR=0.001,
        LR_DECAY_SCHEDULE=[(15, 0.1)]  # at epoch 15, lr->0.1*lr
    )
)
